public static class ExcludeException
{
    ...
    public static IEnumerable<T> ExcludeBy<T, TSelector>(this IEnumerable<T> source, IEnumerable<T> other, Func<T, TSelector> selector)
    {
        if (source is null)
            throw new ArgumentNullException(nameof(source));
